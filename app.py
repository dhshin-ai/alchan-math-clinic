...
def load_system_prompt(student_grade):
    ...
    default_prompt = (
        ...
        "★ [📈 Desmos(데스모스) 그래프 연동 수칙]:\n"
        "- 함수, 이차방정식, 원의 방정식, 산포도 등 그래프 개형이나 시각화가 필요한 문제가 나오면 절대로 텍스트나 코드로 직접 그리려 하지 마라.\n"
        "- 학생이 클릭해서 직접 그래프를 그려보고 조작할 수 있도록 데스모스 링크를 마크다운 형태로 제공해라.\n"
        "- **링크 형식:** [📈 데스모스 공학용 계산기 열기](https://www.desmos.com/calculator)\n"
        "- **안내 방식:** 링크와 함께 '위 링크를 누르고 입력창에 `y = (x-2)^2 - 1`을 적어서 꼭 눈으로 모양을 확인해 봐!' 형태로 행동 지침을 짧게 1줄로 던져라.\n\n"
